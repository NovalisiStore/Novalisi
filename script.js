...
// Paste your Google Apps Script Web App URL here after setup (see README comment below)
// HOW TO SET UP:
//   1. Open your Google Sheet → Extensions → Apps Script
//   2. Paste the doPost script (provided separately) → Save → Deploy as Web App
...
let gridGenre = "";
let catGenre = "";
let catSort = "";   // "", "price-asc", "price-desc", "title-asc", "discount"
...
function showPage(name) {
    ...
    // Always show nav solid when switching pages
    const nav = $("main-nav");
...
function renderHeroBooks() {
    ...
    el.innerHTML = allBooks.slice(0, 4).map(b => {
        const p = b._palette;
        return `<div class="hero-book" onclick="openBook(${b._id})" style="background:${p.bg}">
...
function renderCatalog() {
    ...
    // Update sort select options text (language-aware)
    const sel = $("sort-select");
    if (sel) {
...
function isValidPhone(val) {
    // Accepts formats like +995 555 000 000 / 0555000000 / +1-800-555-0199 etc.
...
// Live validation: clear error as user types valid value
...
async function submitForm() {
    ...
    if (name.length < 2) {
        const msg_ka = "გთხოვთ შეიყვანოთ სახელი";
        const msg_en = "Please enter your name";
        setFieldError("f-name", "f-name-err", currentLang === "ka" ? msg_ka : msg_en);
        hasError = true;
    } else {
        clearFieldError("f-name", "f-name-err");
    }
    ...
    if (!cont) {
        const msg_ka = "გთხოვთ შეიყვანოთ ელ-ფოსტა ან ტელეფონი";
        const msg_en = "Please enter your email or phone";
        setFieldError("f-contact", "f-contact-err", currentLang === "ka" ? msg_ka : msg_en);
        hasError = true;
    } else if (!isValidContact(cont)) {
        const msg_ka = "შეიყვანეთ სწორი ელ-ფოსტა (user@mail.com) ან ტელეფონი (+995…)";
        const msg_en = "Enter a valid email (user@mail.com) or phone (+995…)";
        setFieldError("f-contact", "f-contact-err", currentLang === "ka" ? msg_ka : msg_en);
        hasError = true;
    } else {
        clearFieldError("f-contact", "f-contact-err");
    ...
    if (hasError) {
        // Scroll the first errored field into view smoothly
    ...
    if (FORM_WEBHOOK_URL) {
        try {
            ...
            console.log("✅ Request sent (should appear in sheet now)");
...
(function () {
    ...
    const SHOW_DELTA    = 8;     // px scrolled up before we show again
